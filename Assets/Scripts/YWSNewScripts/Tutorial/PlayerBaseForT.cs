...
public class PlayerBaseForT : MonoBehaviour
{
    // キーネーム
    ...
    protected void InputSkill()
    {
        ...
        //}
        if (TutorialDirector.Instance.tutorialPhase == TutorialDirector.TutorialPhase.SkillActive && Input.GetKeyDown(KeyCode.C) || TutorialDirector.Instance.tutorialPhase == TutorialDirector.TutorialPhase.SkillActive && _DS4_square_value)
        {
    ...
    protected void PieceMove()
    {
        ...
        // 移動後の座標に障害物がなければ
        if (TutorialMap.Instance.CheckWall(movedPos) && TutorialMap.Instance.CheckWall(rotMovedPos))
        {
        ...
        {
            TutorialDirector.Instance.gameState = TutorialDirector.GameState.confirmed;
        }
    }
    ...
    protected void PieceRotate()
    {
        ...
        // 壁にあたってない時
        if (TutorialMap.Instance.CheckWall(rotatedPos))
        {
            // 回転Posが-1かつ下のコマが壁にあたっている時
            if ((int)rotatedPos.z == -1 && !TutorialMap.Instance.CheckWall(rotatedUnderPos))
                rotationNum = lastNum;
            else
            {
                // 回転番号と前の値が違う時 
        ...
        else
        {
            ...
            if (!TutorialMap.Instance.CheckWall(rotatedLeftPos) && !TutorialMap.Instance.CheckWall(rotatedRightPos))
                // 回転入力でクイックローテート
                QuickRotate();
            else
    ...
    private void AnotherTurn(Vector3 rotatedLeftPos, Vector3 rotatedRightPos)
    {
        ...
        if ((rotationNum == 0 || rotationNum == 2) && !TutorialMap.Instance.CheckWall(rotatedLeftPos))
        {
        ...
        else if ((rotationNum == 0 || rotationNum == 2) && !TutorialMap.Instance.CheckWall(rotatedRightPos))
        {
    ...
    protected void PrePieceMove()
    {
        ...
        if (move != Vector3.zero)
        {
            ...
            while (true)
            {
                ...
                // 移動後の座標の1つ下に障害物がなければ
                if (TutorialMap.Instance.CheckWall(movedUnderPos) && TutorialMap.Instance.CheckWall(rotMovedPos))
                {
                    controllPiece1.transform.position = movedPos;
    ...
    protected void DebugGameEnd()
    {
        ...
        {
            Debug.Log("GameEnd");
            TutorialDirector.Instance.gameState = TutorialDirector.GameState.end;
        }
    }
    ...
    private bool CheckColor(string type)
    {
        ...
        for (int j = 1; j < 9; j++)
        {
            if (TutorialMap.Instance.map[i, j] == type)
                isThere = true;
        }
    ...
    private bool ActivateCheck(TutorialDirector.GameState targetState, int cost)
    {
        if (TutorialDirector.Instance.gameState == targetState && reversedCount >= cost)
            return true;
        else
            return false;
    }
    ...
    {
        if (TutorialDirector.Instance._isSkillBlack)
            return true;
        else
            return false;
    }
    ...
    {
        if (myColor == TutorialMap.Instance.black)
        {
            return true;
        }
        else
    ...
    private void AddSkillScore(int multiplyNum, int colorCount)
    {
        ...
        if (PlayerCheck())
        {
            TutorialDirector.Instance.AddScore(true, addAns);
            _isSkillBlack = true;
        }
        else if (!PlayerCheck())
        {
            TutorialDirector.Instance.AddScore(false, addAns);
            _isSkillWhite = true;
        }
    }
    ...
    private void NormalSkillFlag()
    {
        if (PlayerCheck())
        {
            TutorialDirector.Instance._isSkillBlack = true;
        }
    }
    ...
    {
        // スキルが発動できない時
        if (!ActivateCheck(TutorialDirector.GameState.preActive, cost) ||
            TutorialMap.Instance.isSkillCheck ||
            NormalSkillCheck())
        {
            return;
        }
        ...
        if (CheckColor(enemyColor))
        {
            ...
            while (true)
            {
                ...
                if (TutorialMap.Instance.map[z, x] == enemyColor)
                {
                    TutorialMap.Instance.map[z, x] = myColor;
                    TutorialMap.Instance.pieceMap[z, x].GetComponent<Piece>().SkillReverse(true);
                    // 検索、リバース処理を行う
                    TutorialMap.Instance.TagClear();
                    TutorialMap.Instance.isSkillActivate = true;
                    TutorialDirector.Instance.gameState = TutorialDirector.GameState.idle;
                    StartCoroutine(TutorialMap.Instance.CheckReverse(TutorialMap.Instance.pieceMap[z, x], true));
    ...
    {
        if (!ActivateCheck(TutorialDirector.GameState.preActive, cost) &&
            !ActivateCheck(TutorialDirector.GameState.active, cost) ||
            NormalSkillCheck())
        {
            return;
        }
        ...
        if (CheckColor(myColor))
        {
            ...
            while (true)
            {
                ...
                int z = Random.Range(2, 9);
                int x = Random.Range(1, 9);
                if (TutorialMap.Instance.map[z, x] == myColor)
                {
                    TutorialMap.Instance.map[z, x] = myColorfixity;
                    TutorialMap.Instance.pieceMap[z, x].GetComponent<Piece>().ChangeIsFixity();
    ...
    public void Cancellation(int cost)
    {
        ...
        if (CheckColor(enemyColorfixity))
        {
            ...
            for (int z = 2; z < 9; z++)
            {
                ...
                {
                    // 相手の固定こまがあったら
                    if (TutorialMap.Instance.map[z, x] == enemyColorfixity)
                    {
                        if (!isCheck)
                        {
                            isCheck = true;
                            // 色を戻す
                            TutorialMap.Instance.map[z, x] = enemyColor;
                            TutorialMap.Instance.pieceMap[z, x].GetComponent<Piece>().ChangeIsFixity();
    ...
    {
        while (!TutorialDirector.Instance._isLanding)
        {
        ...
        // 一番上におかれたときに除外
        TutorialMap.Instance.CheckHeightOver(controllPiece1, true);
        TutorialMap.Instance.CheckHeightOver(controllPiece2, true);
    ...
    private void ForceConvertionReverse(int px, int pz)
    {
        ...
        for (int z = pz; z < pz + 3; z++)
        {
            for (int x = px; x < px + 3; x++)
            {
                ...
                // 固定コマを解除
                if (TutorialMap.Instance.map[z - 1, x - 1] == enemyColorfixity)
                {
                    TutorialMap.Instance.map[z - 1, x - 1] = enemyColor;
                    TutorialMap.Instance.pieceMap[z - 1, x - 1].GetComponent<Piece>().ChangeIsFixity();
                }

                // その座標に相手の駒があった場合、自分の色に変更
                if (TutorialMap.Instance.map[z - 1, x - 1] == enemyColor)
                {
                    TutorialMap.Instance.map[z - 1, x - 1] = myColor;
                    TutorialMap.Instance.pieceMap[z - 1, x - 1].GetComponent<Piece>().SkillReverse(false);
                    myColorCount++;
                }
            }
        }
    ...
    private void OneRawReverse(int z)
    {
        ...
        for (int x = 0; x < 9; x++)
        {
            ...
            {
                TutorialMap.Instance.map[z, x] = myColor;
                TutorialMap.Instance.pieceMap[z, x].GetComponent<Piece>().SkillReverse(false);
                myColorCount++;
            }
            else
            {
    ...
    public void PriorityGet(int cost)
    {
        ...
        else if (myColor == TutorialMap.Instance.white)
        {
    ...
    private void PriorityGetReverse(bool black)
    {
        ...
        {
            // 固定コマ解除
            if (TutorialMap.Instance.map[x, z] == enemyColorfixity)
            {
                TutorialMap.Instance.map[x, z] = enemyColor;
                TutorialMap.Instance.pieceMap[x, z].GetComponent<Piece>().ChangeIsFixity();
            }

            if (TutorialMap.Instance.map[x, z] == enemyColor)
            {
                TutorialMap.Instance.map[x, z] = myColor;
                TutorialMap.Instance.pieceMap[x, z].GetComponent<Piece>().SkillReverse(false);
                myColorCount++;
            }
    ...
    public void RobberyMoment(int cost)
    {
        ...
        for (int i = 2; i < 9; i++)
        {
            for (int j = 1; j < 9; j++)
            {
                ...
                if (TutorialMap.Instance.map[i, j] == myColor)
                {
                    TutorialMap.Instance.map[i, j] = enemyColor;
                    TutorialMap.Instance.pieceMap[i, j].GetComponent<Piece>().SkillReverse(false);
                }
                else if (TutorialMap.Instance.map[i, j] == enemyColor)
                {
                    TutorialMap.Instance.map[i, j] = myColor;
                    TutorialMap.Instance.pieceMap[i, j].GetComponent<Piece>().SkillReverse(false);
    ...
    public void TutorialPhaseChange()
    {
        if (TutorialDirector.Instance.isFadeIn == true && (_DS4_circle_value || Input.GetKeyDown(KeyCode.Space)))
        {
            ...
            TutorialDirector.Instance.isFadeIn = false;
